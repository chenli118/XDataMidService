...
namespace XDataMidService.Controllers
{
    ...
    public class XDataController : ControllerBase
    {
        ...
        public Task XData2EAS([FromBody] Models.xfile xfile)
        {
            ...
            {
                st.Push(xfile);
                //System.Collections.Stack mySyncdStack = System.Collections.Stack.Synchronized(st);
                while (!StaticData.X2EasList.ContainsValue(localDbName) && st.Count > 0)
                {
                    lock (st.SyncRoot)
                    {
                        foreach (object item in st)
                        {
                            var xf = item as xfile;
                            response = Real2EasImp(xf);
        ...
        private XDataResponse Real2EasImp(xfile xfile)
        {
            XDataResponse response = new XDataResponse();
